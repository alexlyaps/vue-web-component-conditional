<template>
  <div class="background-overlay" ref="outside" @click="handleClickOutside">
    <div class="wrapper">
      <div class="content">
        <RowComponent direction="row" justifyItems="space-between" alignItems="center" :bottomIndent="24">
          <TextComponent tag="h2" size="display-xs" weight="semibold" align="left" color="gray-700"> {{ title }} </TextComponent>
          <SvgIcon name="x" :size="24" color="gray-900" :bottomIndent="8" @click="handleXClick" />
        </RowComponent>
      ...
      <div class="save-cancel-buttons">
        <ButtonBase caption="Cancel" size="md" variant="secondary_gray" @click="handleCancelClick" />
        <ButtonBase caption="Save" size="md" variant="primary" @click="handleSaveClick" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    recursiveDeleteCondition(conditionId, conditions) {
      for (let i = 0; i < conditions.length; i++) {
        if (conditions[i].type === 'condition' && +conditions[i].id === +conditionId) {
          conditions.splice(i, 1);
          return true;
        } else if (conditions[i].type === 'group') {
    ...
    recursiveSearchAndReplace(conditionId, conditions, inputType, optionValue) {
      for (let i = 0; i < conditions.length; i++) {
        if (conditions[i].type === 'condition' && conditions[i].id === conditionId) {
          ...
          this.recursiveSearchAndReplace(conditionId, conditions[i].conditions, inputType, optionValue);
        }
      }
      return false;
    },
    recursiveSetAllAny(groupId, conditions, optionValue) {
      for (let i = 0; i < conditions.length; i++) {
        if (conditions[i].type === 'group' && conditions[i].id === groupId) {
          ...
          this.recursiveSetAllAny(groupId, conditions[i].conditions, optionValue);
        }
      }
      return null;
    },
    ...
    handleElementSelect(option, conditionId) {
      this.recursiveSearchAndReplace(conditionId, this.userLogic, 'element', option);
    },
    handleOperationsSelect(option, conditionId) {
      this.recursiveSearchAndReplace(conditionId, this.userLogic, 'operation', option);
    },
    handleValueSelect(option, conditionId) {
      this.recursiveSearchAndReplace(conditionId, this.userLogic, 'value', option);
    },
    ...
    handleClick(event) {
      ...
      if (addConditionButton) {
        const groupId = +addConditionButton.closest('[data-group-id]').dataset.groupId;
        this.handleAddConditionClick(groupId);
      } else if (addGroupButton) {
        const groupId = +addGroupButton.closest('[data-group-id]').dataset.groupId;
        this.handleAddGroupClick(groupId);
      } else if (deleteGroupButton) {
        const groupId = +deleteGroupButton.closest('[data-group-id]').dataset.groupId;
        this.handleDeleteGroupClick(groupId);
      } else if (deleteConditionButton) {
        const conditionId = +deleteConditionButton.closest('[data-condition-id]').dataset.conditionId;
